...
const SimRequestAdminpage = ({actioncallset}) => {
    ...
    const getaction=(e)=>{
        ...
        if(e.type === 'edit'){
            ...
            setactiontype(false)
            reqremoveItem(selecteditem).then(res=>{
                if(res === 200){
                    getorderlist()
                    setshowpopup(true)
...
const getselecteditem=(e)=>{
    ...
    requpdateItemstatus(selecteditem,e).then(res=>{
        if(res === 200){
...
useEffect(() => { 
  ...
  return (
                ...
                {dataset?.map((item,index) => (
                    <TableRow key={index} style={{borderColor:'#595959',maxHeight:'auto',height:'10rem'}}>
                        ...
                        <TableCell onClick={()=>setselecteditem(item)} style={{backgroundColor:'',display:'flex',justifyContent:'center',alignItems:'center',position:'relative'}}>
                                 ...
                                 <div style={{width:"8rem",backgroundColor:''}}> <Dropdown getdata={getselecteditem}  order_status={item.status}/></div>
    ...
    </Card>:<Nodatafound btn_text={''}  tittle_head={'No Order List Found'} title_des={'Aliquam porta nisl dolor, molestie pellentesque elit molestie in. Morbi metus neque, elementum ullam'}/>}
