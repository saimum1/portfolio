...
const GroupList = () => {
    ...
    const GetgroupList = async () => {
        try {
            ...
            setgroupList(response.data.SIMCards)
...
const getdataforadd=(e)=>{
  ...
  return (
    ...
    <AddnewGroup  isOpen={isOpen} onClose={onClose} data={selecteditem} action={actiontype}  getdata={getdataforadd}/>
            ...
            <TableBody  >
                ...
                {groupList?.map((item,index) => (
                    <TableRow key={item.id} style={{borderColor:'#595959'}}>
                        ...
                        <TableCell>
                            <Text style={{display:'flex',justifyContent:'center',alignItems:'center',gap:'5px'}}>{item?.role_permissions?.map((n,key)=>{
                                return (<span> {n} {(item?.role_permissions?.length - 1) !== key?<span>,</span>:''}</span> )
                            })}</Text>
                        </TableCell>
